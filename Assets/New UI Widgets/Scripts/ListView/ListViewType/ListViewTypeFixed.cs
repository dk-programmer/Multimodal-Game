namespace UIWidgets
{
		...
		where TItemView : ListViewItem
	{
		...
		protected class ListViewTypeFixed : ListViewTypeRectangle
		{
			...
			{
				return GetItemPosition(index) + (GetInstanceSize(index) / 2f) - (Owner.ScaledScrollRectSize() / 2f);
			}
			...
			{
				return GetItemPosition(index) + GetInstanceSize(index) + Owner.LayoutBridge.GetMargin() - Owner.LayoutBridge.GetSpacing() - Owner.ScaledScrollRectSize();
			}
			...
			public override void CalculateMaxVisibleItems()
			{
				...
				var max = Owner.IsHorizontal()
					? Mathf.CeilToInt(Owner.ScrollRectSize.x / Owner.DefaultInstanceSize.x)
					: Mathf.CeilToInt(Owner.ScrollRectSize.y / Owner.DefaultInstanceSize.y);
